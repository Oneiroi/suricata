        ...
        *alproto = AppLayerProtoDetectGetProto(app_tctx->alpd_tctx,
        ...
        if (*alproto != ALPROTO_UNKNOWN) {
            ...
            if (*alproto_otherdir != ALPROTO_UNKNOWN) {
                 ...
                 * tc direction. */
                } else if (size_tc > 100000 && FLOW_IS_PP_DONE(f, STREAM_TOSERVER) && !(FLOW_IS_PM_DONE(f, STREAM_TOSERVER)) &&
                           FLOW_IS_PM_DONE(f, STREAM_TOCLIENT) && FLOW_IS_PP_DONE(f, STREAM_TOCLIENT)) {
                    FlowSetSessionNoApplayerInspectionFlag(f);
                    StreamTcpSetStreamFlagAppProtoDetectionCompleted(&ssn->server);
                    StreamTcpSetStreamFlagAppProtoDetectionCompleted(&ssn->client);
                 ...
                 * ts direction. */
                } else if (size_ts > 100000 && FLOW_IS_PP_DONE(f, STREAM_TOCLIENT) && !(FLOW_IS_PM_DONE(f, STREAM_TOCLIENT)) &&
